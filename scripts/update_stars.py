"""Generate stars.md from GitHub & Codeberg starred repos.
...
has no list concept, so its section is flat-alphabetized.
...
stars. If unset, the Codeberg section is skipped with a warning.
...
OUTPUT_PATH = os.environ.get("STARS_OUTPUT", "stars.md")
...
def fetch_codeberg_starred(token: str | None) -> list[dict[str, Any]]:
    if not token:
        log("  CODEBERG_TOKEN unset — skipping Codeberg section")
        return []
    out: list[dict[str, Any]] = []
    page = 1
    headers = {
...
def render_section(heading: str, items: list[dict[str, Any]]) -> list[str]:
    if not items:
        return []
    lines = [heading, ""]
...
def build_markdown(
    ...
    codeberg: list[dict[str, Any]],
) -> str:
    lines = [
        "# Stars",
        "",
        "> Auto-generated by `nix run .#update-stars`. Do not edit by hand.",
        "",
        "## GitHub",
        "",
    ]
    ...
    if github_lists is None:
        lines.extend(render_section("### All stars", github_all))
    else:
        in_lists: set[str] = set()
        for lst in github_lists:
            lines.extend(render_section(f"### {lst['name']}", lst["items"]))
            for it in lst["items"]:
                in_lists.add(it["nameWithOwner"])
        uncategorized = [r for r in github_all if r["nameWithOwner"] not in in_lists]
        lines.extend(render_section("### Uncategorized", uncategorized))

    lines.extend(["## Codeberg", ""])
    if codeberg:
...
def main() -> int:
    ...
    markdown = build_markdown(github_lists, github_all, codeberg)

    with open(OUTPUT_PATH, "w", encoding="utf-8", newline="\n") as f:
        f.write(markdown)
    log(f"wrote {OUTPUT_PATH} ({len(markdown)} bytes)")
    return 0
